...
import 'package:tsscourses/presentation/components/widgets/chapitre_widget_web.dart';
import 'package:tsscourses/presentation/components/widgets/lesson_widget.dart';

class LearningScreenWeb extends StatefulHookConsumerWidget {

  Formation formation;
  LearningScreenWeb(this.formation);

  @override
  LearningScreenWebState createState() => new LearningScreenWebState(this.formation);
}

class LearningScreenWebState extends ConsumerState<LearningScreenWeb> {

 Formation formation;
 LearningScreenWebState(this.formation);
 ...
 List<Widget> buildFormation(Formation formation) {
    ...
    for(int i = 0; i < formation.contenus.length; i++) {
        ...
        for(int j = 0; j < formation.contenus[i].chapitres.length; j++) {
            ...
            liste.add(ChapitreWidgetWeb(formation: formation, lesson: formation.contenus[i], item: formation.contenus[i].chapitres[j],));
        }
                          ...
                          ExpansionPanelList(
                            ...
                            children: formation.contenus.map<ExpansionPanel>((Lesson item) {
                              return ExpansionPanel(
                                ...
                                body: Column(
                                  children: item.chapitres.map((e) => ChapitreWidgetWeb(formation: formation, lesson: item, item: e)).toList(),
                                ),
